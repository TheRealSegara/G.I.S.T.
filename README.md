...
An access code unlocks the *device* for a school; a student account is what makes G.I.S.T. useful across more than one sitting. After "Start Playing," a student chooses **New Student** (enter a full name, remembered for next time) or **Returning Student** (re-enter the same name to resume). That account is what a completed session gets saved against, so the teacher's **File Box** (reachable from the main menu's Teachers panel) can list every student who's signed up under this access code and drill into any of their past sessions' full diagnostic reports.

**Students are identified by full name alone, nothing to remember beyond that.** This is a supervised-classroom tool for tracking progress across sessions, not a security boundary against a determined attacker — the database's unique index on (access code, name) already prevents two students under the same code from colliding, so there's no need for anything password-like on top of it. The login/signup endpoint still rate-limits requests server-side, against automated abuse rather than credential guessing.
...
│   ├── _studentAuth.js          # Name normalization, allowed-value lists
