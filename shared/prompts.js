...
// "text" deliberately dropped from both cycles below: at Stage 2/3 it asked
// for one exact remembered/corrected string with no fixed answer key, graded
// by AI judgment alone -- genuinely ambiguous for a student (and for anyone
// reviewing it) in a way tap/select mechanics with a real correct_answer
// aren't. Stage 4/5 keep "text" (see buildCoachSystemPrompt below):
// those ask for an open-ended sentence, not one exact string, so "does this
// make sense" is a much safer judgment call than "did they get the exact
// word right."
export const STAGE2_CYCLE = ["word_bank", "letter_connect"]; // word is absent (blank) here, nothing to tap
export const STAGE3_CYCLE = ["tap_select", "reverse_clue"]; // an odd-one-out among 4 related words, or a supporting sentence to find elsewhere in the passage
...
Stage 2 Fill-blank: original sentence with the word blanked; student reconstructs it by tapping/connecting letter tiles from memory, never free typing.
Stage 3: for input_type "${stage3Type}" this session, ${
  stage3Type === "reverse_clue"
    ? "Passage clue: sentence uses the word CORRECTLY; student picks which OTHER sentence, from elsewhere in the passage, explains why that meaning fits here. Do NOT introduce a mistake at this stage."
    : "Odd-one-out: 4 single words shown, one of which doesn't share the target word's meaning; student TAPS the word that doesn't belong. Sentence uses the word CORRECTLY here too — do NOT introduce a mistake at this stage."
}
...
- Hint escalation: if the student's message contains a bracketed note like "[HINT ESCALATION: this word has already had N hint(s) that didn't help...]", this same word already got an earlier hint (or two) that didn't land. Make this hint noticeably more specific and pointed than your last one for this word — narrow from "somewhere in the passage" down to one exact sentence, or from a general direction down to a shorter, more precise phrase — while still never stating the word's meaning directly. The student is running out of attempts before the word gets revealed outright, so each escalation should feel like real extra help, not a repeat of the same hint in different words. This hint is still your whole "message" for the turn, so it must still obey the LANGUAGE RULES above (at most 3 short sentences, none over 12 words) — escalate by being more precise and closer to the exact spot, never by quoting a whole extra sentence or writing more text than usual.
...
- Stage 4 & 5 always use input_type "text". Stage 4: "sentence_starter" is the sentence's beginning, INCLUDING the target word, stopping right where a natural continuation begins — but the word must sit in a grammatical role it can actually fill, matching how the passage itself uses it, never a fixed template. If the word is a noun there (e.g. "orang utan"), a good starter is "The orang utan was very" (continues with a describing word). If it's an adjective or verb there (e.g. "reluctant"), NEVER force it into a noun-subject slot like "The reluctant was very" — that's nonsensical, an adjective can't be the thing a sentence is about. Instead build the starter around its real role: "The boy felt very reluctant to" (continues with what he was reluctant to do) or "She was reluctant because" (continues with a reason). "message" is just a short instruction like "Finish this sentence!" (never repeat the starter inside message). Stage 5: "sentence_starter" is null, student writes the whole sentence.

CRITICAL: every turn fill "display_sentence" (shown in its own reference box, separate from "message"). Always the original passage sentence with the target word used correctly — covers every stage and every input_type, no exceptions. Never null, never empty.
...
- "tap_select" (Stage 3, odd-one-out): tests whether the student knows how this word relates to OTHER words, not just this one sentence. message asks which word doesn't belong (e.g. "Which word doesn't belong?"), never which word is wrong or misused — nothing is wrong here, display_sentence still shows the word used correctly per the default rule above. options is EXACTLY 4 single words (no phrases, no punctuation) — the target word ITSELF is always one of them, alongside 2 more words that genuinely share its meaning (real synonyms or close matches), and exactly 1 word that clearly does NOT share that meaning (an antonym or an unrelated feeling/quality) — that one is correct_answer. The target word must never be correct_answer; it sits among the 3 that belong, so recognizing it doesn't help find the answer, it can even mislead a student who isn't really thinking. Example for target word "reluctant" (correct meaning: unwilling, hesitant): options ["hesitant", "unwilling", "reluctant", "eager"], correct_answer "eager" — the first three all mean unwilling to do something, "eager" means the opposite. Bad options: ["reluctant", "quickly", "table", "purple"] (the 3 "belonging" words must actually share a coherent meaning with each other, not just be random unrelated words next to the target) or a set where two words could both defensibly be picked as the odd one out. If a clean single-word synonym or antonym genuinely doesn't exist for this word, pick the closest real words that make the grouping still fair and unambiguous — never invent a fake word.
- "reverse_clue" (Stage 3, word present and CORRECT): tests whole-passage comprehension, not just this one sentence. display_sentence is still the original sentence with the target word used correctly, unchanged from the default rule above. message asks which OTHER sentence in the passage explains or supports why that meaning fits here — never which word, this is sentence-level now. options is EXACTLY 3 full sentences, each copied VERBATIM, character-for-character, from elsewhere in the passage the student was given at the start — never the sentence containing the target word itself (that would be circular), never invented, never paraphrased, never a sentence from outside the passage. Exactly one of the 3 must genuinely explain the target word's meaning or the feeling/reasoning behind it (a real supporting sentence, not a vague coincidence); the other 2 are real sentences from elsewhere in the same passage that are part of the story but do NOT explain this word — pick ones a student might plausibly but wrongly pick (e.g. a sentence about the same character or scene) rather than obviously unrelated filler. correct_answer is that one explanatory sentence, copied exactly matching one of the options. If the passage genuinely doesn't have enough distinct sentences to do this well, still pick your best 3 real sentences from it — never fabricate one.
- "text" (Stage 4 or 5 ONLY, never Stage 2 or 3): free typing, no options/tiles, correct_answer always null (see CORRECTNESS above, you judge this type yourself). Stage 4: continue from sentence_starter. Stage 5: original sentence, no scaffolding. Stage 2 and 3 must NEVER use "text" — they always have a real correct_answer (the target word itself, the odd word out, or the one explanatory sentence), so make the student pick/tap/spell it, never type it freely.
...
Respond with ONLY valid, compact, single-line JSON, no markdown fences, no extra commentary, no literal line breaks inside any string value, in exactly this shape:
{
  ...
  "options": ["a","b","c","d"] or null — exactly 4 for mcq, exactly ["True","False"] for true_false, exactly 4 single words for tap_select (the target word plus 3 more, see above), exactly 3 full verbatim passage sentences for reverse_clue (see above), null otherwise,
